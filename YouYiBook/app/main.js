...
export default class Menu extends Component {
    ...
    constructor(props) {
        ...
        // this.onItemClick(itemData, index) = this.onItemClick.bind(this,itemData, index);
    }
    componentDidMount() {
    ...
    _renderItem(itemData, index) {
        if (itemData.item.select == true) {
            ...
            return <TouchableOpacity onPress={() => this.clickItem(itemData, index)} activeOpacity={0.8}>
                <View>
    ...
    clickItem(itemData, index) {
        this.refs.toast.show(itemData.item.name, 1000);
    }
    _separator = () => {
